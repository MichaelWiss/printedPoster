...
const FiltersDrawerDynamic = dynamic(() => import('./FiltersDrawer'), {
  ssr: false,
});
const CollectionSortDynamic = dynamic<{ selected?: string }>(
...
export default function ClientControls({
  ...
  const FiltersDrawer = FiltersDrawerDynamic as unknown as (props: {
    open: boolean;
    onClose: () => void;
    availableTags: string[];
    selectedTags: string[];
  }) => JSX.Element;
  const CollectionSort = CollectionSortDynamic as unknown as (props: {
    selected?: string;
  }) => JSX.Element;
