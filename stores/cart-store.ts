...
const convertFromServerFormat = (serverItems: ServerCartItem[], products: ShopifyProduct[]): CartLineItem[] => {
      ...
      (set, get) => ({
        ...
        addItem: async (product: ShopifyProduct, quantity = 1) => {
          ...
          try {
            ...
            console.log('Updating cart state with new items:', newItems.length, 'items')
        ...
        syncWithServer: async () => {
          ...
          try {
            ...
            if (!get().serverCartId) {
              ...
              const cart = await cartService.createUserCart(get().userId!, serverFormat)
              set({
                serverCartId: cart.id,
                lastSynced: new Date(),
                items: get().items.map(item => ({ ...item, pendingSync: false }))
              })
            } else {
        ...
        migrateGuestCart: async (userId: string) => {
          try {
            ...
            try {
              ...
              const cart = await cartService.createUserCart(userId, serverFormat)
              ...
              set({
                ...
                lastSynced: new Date(),
                items: guestItems.map(item => ({ ...item, pendingSync: false }))
              })
              ...
              set({
                error: 'Failed to migrate your cart. Your items are still safe locally.'
              })
            }
          } catch (error) {
...
export const useIsAuthenticated = () => useCartStore(state => state.isAuthenticated)
export const useAuthUser = () => useCartStore(state => ({ userId: state.userId, isAuthenticated: state.isAuthenticated }))
